...
public class AssemblyLineRecipe extends Recipe {

    private final List<ResearchInputItem> researchInputItems;

    public AssemblyLineRecipe(List<CountableIngredient> inputs,
                            ...
                            int EUt,
                            boolean hidden,
                            List<ResearchInputItem> researchInputItems) {

        super(inputs, outputs, chancedOutputs, fluidInputs, fluidOutputs, recipeProperties, duration, EUt, hidden);

        this.researchInputItems = ImmutableList.copyOf(researchInputItems);
    }
    ...
    public boolean matches(boolean consumeIfSuccessful, List<ItemStack> inputs, List<FluidStack> fluidInputs, MatchingMode matchingMode) {
        ...
        if (matchingMode == MatchingMode.IGNORE_FLUIDS) {
            if (getInputs().isEmpty() && getResearchInputItem().isEmpty()) {
                return false;
            }
        } else {
    ...
    public List<ResearchInputItem> getResearchInputItem() {
        return researchInputItems;
    }
}